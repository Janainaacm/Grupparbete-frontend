...
import AddToCartButton from "../../recipeDetails/components/AddToCartButton";
import { CocktailInterface } from "../../../Types";
import { FaAngleDown, FaAngleUp } from "react-icons/fa";
import DisplayReviews from "../../recipeDetails/components/DisplayReviews";
import PostReview from "../../recipeDetails/components/PostReview";
...
const CocktailDetails = () => {
  ...
  const [recericList, setRecericList] = useState<CocktailInterface[]>([]);
...
const getRecericList = () => {
  const filteredRecipes = cocktailList.filter((item) => item.idDrink !== cocktailToRender.idDrink);
  const shuffledRecipes = shuffle(filteredRecipes).slice(0, 6);
  setRecericList(shuffledRecipes);
}

const chooseFromRecericList = (id: string) => {
  updateCocktailID(id)
      ...
      <div className="recipe-details-header-grid-wrapper">
        <div className="title-side-header">
          <div className="title-side-header-content">
            <div onClick={() => navigate("/Cocktails")} className="reciric-list-all-recipes-button" style={{fontSize: '12px'}}>
            ...
            <div>
              <button className="submit-rating-button-div" onClick={() => AddToCocktailCart(cocktailToRender)}>Add to cart</button>
            </div>
          </div>
        </div>
            ...
            <div className="ingredients-container">
              ...
              <div className="ingredients-list-group">
                <div className="ingredients-list-grid">
                  <p className="ingredient-name">{cocktailToRender.strIngredient1}</p>
                  <p className="ingredient-name">{cocktailToRender.strIngredient2}</p>
                  <p className="ingredient-name">{cocktailToRender.strIngredient3}</p>
                  <p className="ingredient-name">{cocktailToRender.strIngredient4}</p>
                  <p className="ingredient-name">{cocktailToRender.strIngredient5}</p>
                  <p className="ingredient-name">{cocktailToRender.strIngredient6}</p>
                  <p className="ingredient-name">{cocktailToRender.strIngredient7}</p>
                  <p className="ingredient-name">{cocktailToRender.strIngredient8}</p>
                  <p className="ingredient-name">{cocktailToRender.strIngredient9}</p>
                  <p className="ingredient-name">{cocktailToRender.strIngredient10}</p>
                </div>
              </div>
            </div>


          <div className="recipe-details-side-grid" style={{marginRight: '0px'}}>
            <div className="reviews-container" style={{width: '100%'}}>
              <Button
                ...
                aria-expanded={open}
              >
                <h3 className="reviews-title">Instruktioner {toggleDropDown}</h3>
                {""}
              </Button>
              <Collapse in={open}>
                <div id="reviews-inside-collapse-rd">
                    <div className="instructions-list-item">
                      <p className="instruction-title">{cocktailToRender.strInstructions}</p>
                    </div>                </div>
              </Collapse>
            </div>
          </div>
          ...
          <ul className="reciric-list-ul">
            {recericList.map((item, index) => (
                <li className="reciric-list-item">
                  <div onClick={() => chooseFromRecericList(item.idDrink)} className="receric-list-title-a">
                  <span className="receric-list-item-image">
                    <img className="receric-list-img" src={item.strDrinkThumb} alt={item.strDrink} />
                  </span>
                  <div className="receric-list-overlay">
                    <div className="receric-list-overlay-grid">
                    <h4 className="receric-list-overlay-title">{item.strDrink}</h4>
                    <p className="receric-list-overlay-categories">{item.strCategory}</p>
                    </div>
                  </div>
                  </div>
                </li>
